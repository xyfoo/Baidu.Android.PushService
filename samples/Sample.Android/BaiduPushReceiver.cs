    ...
    public class BaiduPushReceiver: PushMessageReceiver
    {
        ...
        private async void SendRegistrationToAppServer(string userId, string channelId)
        {
            ...
            await Task.Run(() =>
            {
                ...
                var hub = new NotificationHub("AzureNotificationHubName", "AzureNotificationHubAccessPolicyConnectionString", Application.Context);
                var regId = hub.RegisterBaidu(userId, channelId);
                System.Diagnostics.Debug.WriteLine($"Azure Notification Hub registation successful: {regId} for userid: {userId}, channelId: {channelId}");
            });
        }
